...
public enum Ec2Region {
    ...
    static final Map<Ec2Region, String> enumToString = new HashMap<Ec2Region, String>() {{
    ...
    static final Map<String, Ec2Region> stringToEnum = new HashMap<String, Ec2Region>() {{
        for (Entry<Ec2Region, String> entry : enumToString.entrySet()) {
            put(entry.getValue(), entry.getKey());
        }
    }};

    public static Ec2Region fromString(String string) {
        Ec2Region geoLocation = stringToEnum.get(string);
        if (geoLocation != null) {
            return geoLocation;
        }
